...
This file is the working guide for agents editing `alibaba-repos`.

## What this repo is

`alibaba-repos` is a single-page static catalog of public repositories from
Alibaba's `alibaba` GitHub organization. It follows a
...
- `cloudflare/worker.js`: proxy used for
  `https://glaubi.net/alibaba`.

## Catalog rules

Included repositories must be:

- Owned by the `alibaba` GitHub org.
...
2. Data, Databases & Analytics
3. Cloud, Microservices & Middleware
4. Languages, Libraries & Developer Tools
5. Web, Mobile & User Interfaces
6. Systems, Networking & Observability
7. Research, Vision & Media
...
That command queries GitHub for current `alibaba` repositories
matching the catalog rules, fetches metadata and recent commit counts, assigns
each repo to a rough research cluster, rebuilds the fresh-traction table,
